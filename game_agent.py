...
def custom_score(game, player):
    ...
    return (float(my_moves - 2 * opp_moves) + next_next_moves / mahattan_centre_score)
    """
        ...
        **********************************************************************
        ...
        if game_in_terminal_state(game, self) or depth <= 0:
            ...
            def minimizer(game, depth):
                ...
                if game_in_terminal_state(game, self) or depth <= 0:
                    best_move, best_value = game.get_player_location(self), self.score(game, self)
                else:
                    best_value = float("+inf")
                    for move in game.get_legal_moves(game.active_player):
                        next_ply = game.forecast_move(move)
                        score = maximizer(next_ply, depth - 1)[1]
                        if min(best_value, score) == score:
                            best_move = move
                            best_value = score
            ...
            def maximizer(game, depth):
                ...
                if game_in_terminal_state(game, self) or depth <= 0:
                    best_move, best_value = game.get_player_location(
        ...
        if game_in_terminal_state(game, self):
            ...
            def minimizer(game, depth, alpha, beta):
                ...
                if game_in_terminal_state(game, self) or depth <= 0:
                    ...
                    best_value = float("+inf")
                    #print("Legal Moves: {}".format(game.get_legal_moves(game.active_player)))
                    for move in game.get_legal_moves(game.active_player):
                        next_ply = game.forecast_move(move)
                        _, value = maximizer(next_ply, depth - 1, alpha, beta)
                        if min(value, best_value) == value:
                            best_value = value
                            best_move = move
